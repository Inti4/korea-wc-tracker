...
function sampleScore(m) {
    if (m.hs != null && m.hs !== "" && m.as != null && m.as !== "")
        return [Number(m.hs), Number(m.as)];
    if (MODEL === "poisson") {
...
function sampleGroupThird(g) {
    const data = groups[g], t = {};
    for (const n in data.teams)
        t[n] = { ...data.teams[n] };
...
function computeGroupFinal(g) {
    // clone teams, apply any entered scores
    const data = groups[g];
    const t = {};
    for (const name in data.teams)
        t[name] = { ...data.teams[name] };
    for (const m of data.matches) {
...
function allMatchesEntered(g) {
    return groups[g].matches.every(m => m.hs != null && m.hs !== "" && m.as != null && m.as !== "");
}
function anyMatchEntered(g) {
    return groups[g].matches.some(m => m.hs != null && m.hs !== "" && m.as != null && m.as !== "");
}
function thirdBeatsKorea(rec) {
    if (rec.pts !== KOREA.pts)
        return rec.pts > KOREA.pts;
    if (gd(rec) !== KOREA.gd)
        return gd(rec) > KOREA.gd;
...
function groupThirdDist(g) {
    const data = groups[g];
    const base = {};
    for (const name in data.teams)
        base[name] = { ...data.teams[name] };
    const outcomes = [];
    function rec(i, tbl) {
...
function overallProb() {
    ...
    for (let s = 0; s < N; s++) {
        ...
        for (const g of liveGroups)
            if (thirdBeatsKorea(sampleGroupThird(g)))
                above++;
        if (above + 1 <= 8)
            inCount++; // Korea herself is the +1; needs ≤8 total
    }
...
function verdictState() {
    ...
    for (const g in groups) {
        if (allMatchesEntered(g)) {
            if (thirdBeatsKorea(computeGroupFinal(g).ranked[2])) {
                minAbove++;
                maxAbove++;
            }
        }
            ...
            if (outs.some(thirdBeatsKorea))
                maxAbove++; // some outcome puts a team above
            if (outs.every(thirdBeatsKorea))
                minAbove++; // every outcome does → unavoidable
        }
    }
    const status = maxAbove <= 7 ? "clinched" : minAbove >= 8 ? "eliminated" : "alive";
...
function renderTop8() {
    ...
    const all = [...locked, ...live].sort((a, b) => b.pts - a.pts || (b.gf - b.ga) - (a.gf - a.ga) || b.gf - a.gf);
    let html = "";
    all.forEach((t, i) => {
        const rank = i + 1;
        const inTop8 = rank <= 8;
      ...
      <td>${status}</td>
        ...
        if (rank === 8)
            html += `<tr class="cutline"><td colspan="6">— top 8 cut line —</td></tr>`;
    });
    box.innerHTML = html;
}
...
function resetScores() {
    for (const g in groups)
        groups[g].matches.forEach(m => {
            if (m.confirmed)
                return; // never wipe a real, locked-in result
            m.hs = "";
            m.as = "";
...
function recordProb(p) {
    const sig = stateSignature();
    if (sig === lastStateSig)
        return; // unchanged input → don't record MC noise
    lastStateSig = sig;
    probHistory.push(p);
    if (probHistory.length > 40)
        probHistory.shift();
    drawSparkline();
}
...
function toggleModel() {
    ...
    if (b)
        b.textContent = MODEL === "uniform" ? 'Model: Uniform 0–4' : 'Model: Strength-weighted';
    render();
}
// ============================================================
...
let lastVerdictStatus = null;
const prevApiScores = {}; // key "G:0" → last score
function enableNotifications() {
    ...
    Notification.requestPermission().then(perm => {
        ...
        if (b)
            b.textContent = notifyEnabled ? "🔔 Notifications on" : "🔕 Notifications blocked";
    });
}
...
function maybeNotifyVerdict(status) {
    if (lastVerdictStatus && status !== lastVerdictStatus && status !== "alive") {
        notify(status === "clinched" ? "✅ Korea is THROUGH!" : "⛔ Korea is OUT", "The third-place picture just changed.");
    }
...
function matchesHint(apiName, hints) {
    ...
    try {
        ...
        for (const g in groups) {
            groups[g].matches.forEach((m, i) => {
                ...
                const found = apiGames.find(game => matchesHint(game.home_team_name_en, homeHints) &&
                ...
                if (haveScore) {
                    const newH = parseInt(hs, 10), newA = parseInt(as, 10);
...
function anyMatchLiveSoonOrNow() {
    const now = new Date();
    for (const g in groups) {
        for (const m of groups[g].matches) {
            if (!m.kickoff)
...
function scheduleNextAutoRefresh(matchInProgress, isErrorBackoff) {
    if (liveTimer)
